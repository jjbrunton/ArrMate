...
├── Quality Check Job (Arr instances, every 5 min)
│   ├── Fetch cutoff-unmet items from Sonarr/Radarr
│   ├── Select items whose latest known search time is older than 24 hours
│   └── Queue upgrade searches for up to the instance-configured batch size
