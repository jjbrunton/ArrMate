...
| POST | `/api/instances/[id]/cutoff/search` | Trigger an upgrade search for selected cutoff-unmet items that have not been searched in the last 24 hours; items that have already met their quality cutoff are returned in `cutoffMetIds` and excluded from searching, remaining skipped item IDs are returned so the UI can explain local cooldown no-ops, and the route returns 409 when ArrMate already has a quality search running or Sonarr/Radarr already has active search commands in its own command queue |
